...
const Ready = () => {
  ...
  const {
    ...
    enrollUserOneFa,
    progress: progressOneFa,
    faceDetected: faceDetectedOneFa,
  } = useEnrollOneFa("userVideo", useEnrollSuccess, null, deviceId);

  const handeEnrollOneFa = () => {
  ...
  return (
    <div id="canvasInput" className="container">
      ...
      >
        ...
        <select defaultValue={device} onChange={(e) => handleSwitchCamera(e)}>
          {devices.map((e, index) => {
            return (
        ...
        {currentAction === "useEnrollOneFa" && (
          <div>
            <div>
              Enroll 1FA Face Detected:
              {faceDetectedOneFa ? "Face Detected" : "No Face Detected"}
            </div>
            <div> Enroll Status: {enrollOneFaStatus} </div>
            <div> Progress: {`${progressOneFa} %`}</div>
            <div>Enroll UUID: {`${enrollOneFaData ? enrollOneFaData.PI.uuid : ""}`}</div>
            <div>Enroll GUID: {`${enrollOneFaData ? enrollOneFaData.PI.guid : ""}`}</div>
          </div>
        )}
